...
- Установка закреплённой версии KEDA в namespace `keda`.
...
ansible-playbook -i ansible/inventory.yml ansible/playbooks/install-keda.yml
ansible-playbook -i ansible/inventory.yml ansible/playbooks/deploy-auth-service.yml
...
Отключить автомасштабирование Code Runner и удалить KEDA:

```bash
ansible-playbook -i ansible/inventory.yml ansible/playbooks/uninstall-keda.yml
```

Playbook оставляет KEDA CRD в кластере, чтобы последующая установка не требовала их восстановления.
